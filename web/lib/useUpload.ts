...
import { UploadApiClient, uploadChunk } from '@shared/api/upload';
import { SUPPORTED_VIDEO_TYPES, MAX_FILE_SIZE, MIN_FILE_SIZE } from '@shared/constants/upload';
import { calculateChunkSize } from '@shared/utils/upload';
...
interface UploadProgress {
  ...
  fileSize: number;
  progress: number;
  status: 'uploading' | 'finalizing' | 'completed' | 'failed' | 'paused';
...
export function useUpload(): UseUploadReturn {
  ...
  const apiClient = new UploadApiClient(
    ...
    try {
      ...
      const uploadChunkWithProgress = async (chunkIndex: number): Promise<void> => {
        ...
        try {
          ...
          await uploadChunk(chunkUrls![chunkIndex], chunkData, (chunkProgress) => {
